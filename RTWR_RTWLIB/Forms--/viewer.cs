    ...
    public partial class EDU_viewer : Form
    {
        ...
        public void UpdateUnitTxt(string command = null, string lookup = null)
        {
            ...
            foreach (string line in linedOutput)
            {
                ...
                {
                    if (EDU.edu_scheme.Scheme.ContainsKey(ExtString.GetFirstWord(line.Trim())))
                    {
                        Dictionary<string, int> components = EDU.edu_scheme.GetComponents(ExtString.GetFirstWord(line.Trim()));
                ...
                if (marked.Count > 0)
                {
                    string space = spaces(ExtString.GetFirstWord(line), 25);
                    rtxt_unit.AppendText(ExtString.GetFirstWord(line) + space);
                    string firstRemoved = ExtString.RemoveFirstWord(line);
                ...
                else
                {
                    string space = spaces(ExtString.GetFirstWord(line), 25);
                    rtxt_unit.AppendText(ExtString.GetFirstWord(line) + space);
                    rtxt_unit.AppendText(ExtString.RemoveFirstWord(line) + "\r\n");
